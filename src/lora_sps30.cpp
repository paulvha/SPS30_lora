 ...
 */
bool lora_SPS30::begin(serial_port port)
{
    _Sensor_Comms = port;
    ...
    else     // setup serial communication
    {
     ...
     if (_Sensor_Comms == SERIALPORT1)
    {
        Serial1.begin(115200);
        _serial = &Serial1;     // LORA default ONLY serial1
        delay(1000);
    }
    else {
        printf("only serial1 is supported\n");
        return(false);
    }
 ...
 */
void lora_SPS30::EnableDebugging(uint8_t act)
{
    _SPS30_Debug = act;
}
 ...
 *  false = error
 */
bool lora_SPS30::Instruct(uint8_t type)
{
    uint8_t ret;
 ...
 *  else error
 */
uint8_t lora_SPS30::Get_Device_info(uint8_t type, char *ser, uint8_t len)
{
...
uint8_t lora_SPS30::SetAutoCleanInt(uint32_t val)
{
 ...
 */
/* set the pin to use */
void lora_SPS30::SetSerialPin(uint8_t rx, uint8_t tx)
{
    Serial_RX = rx;
    Serial_TX = tx;
}
 ...
 *  else error
 */
uint8_t lora_SPS30::GetValues(struct sps_values *v)
{
    uint8_t ret, loop;
    uint8_t offset;
    ...
    // get data (reduced for Lora implementation due to memory constrains)
 ...
 * return : float number
 */
float lora_SPS30::byte_to_float(int x)
{
    ByteToFloat conv;
 ...
 */
uint32_t lora_SPS30::byte_to_U32(int x)
{
    ByteToU32 conv;
 ...
 */
int lora_SPS30::ByteStuff(uint8_t b, int off)
{
    uint8_t  x = 0;
 ...
 */
uint8_t lora_SPS30::ByteUnStuff(uint8_t b)
{
    switch(b){
 ...
 *  false ERROR
 */
bool lora_SPS30::SHDLC_fill_buffer(uint8_t command, uint32_t parameter)
{
 ...
 */
uint8_t lora_SPS30::SHDLC_calc_CRC(uint8_t *buf, uint8_t first, uint8_t last)
{
    uint8_t i;
    uint32_t ret = 0;
 ...
 *   else error
 */
uint8_t lora_SPS30::SendToSerial()
{
    uint8_t i;
...
uint8_t lora_SPS30::ReadFromSerial()
{
    ...
    {
        if (_SPS30_Debug) printf("CRC error\n");
        return(ERR_PROTOCOL);
    }
 ...
 */
uint8_t lora_SPS30::SerialToBuffer()
{
    uint32_t startTime;
 ...
 *
 */
void lora_SPS30::I2C_fill_buffer(uint16_t cmd, uint32_t interval)
{
    memset(_Send_BUF,0x0,sizeof(_Send_BUF));
    _Send_BUF_Length = 0;
 ...
 * else error
 */
uint8_t lora_SPS30::I2C_SetPointer()
{
    if (_Send_BUF_Length == 0) return(ERR_DATALENGTH);
 ...
 *
 */
uint8_t lora_SPS30::I2C_SetPointer_Read(uint8_t cnt, bool chk_zero)
{
    uint8_t ret;
 ...
 * else error
 */
uint8_t lora_SPS30::I2C_ReadToBuffer(uint8_t count, bool chk_zero)
{
    uint8_t data[3];
    uint8_t i, j;
 ...
 *  false if not
 */
bool lora_SPS30::I2C_Check_data_ready()
{
 ...
 * return CRC
 */
uint8_t lora_SPS30::I2C_calc_CRC(uint8_t data[2])
{
    uint8_t crc = 0xFF;
    for(int i = 0; i < 2; i++) {
        crc ^= data[i];
