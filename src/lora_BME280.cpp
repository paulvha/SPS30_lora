 ...
 * @file lora_BME280.h
 *
 *  based on the original  Adafruit_BME280.cpp
 *
 * Reduced version only for I2C
 *
 * Paulvha: september 2019 version 1.0
 *
 * Orignal header below is still valid
 *
 */
 ...
 */
bool lora_BME280::begin(uint8_t addr) {
  _i2caddr = addr;
  _wire = &Wire;
  return init();
}
 ...
 */
void lora_BME280::setSampling(sensor_mode mode,
 ...
 */
void lora_BME280::write8(byte reg, byte value) {
 ...
 */
uint8_t lora_BME280::read8(byte reg) {
  uint8_t value;
 ...
 */
uint16_t lora_BME280::read16(byte reg) {
  uint16_t value;
 ...
 *   @returns the 16 bit data value read from the device
 */
uint16_t lora_BME280::read16_LE(byte reg) {
 ...
 *   @returns the 16 bit data value read from the device
 */
int16_t lora_BME280::readS16(byte reg) { return (int16_t)read16(reg); }
 ...
 *   @returns the 16 bit data value read from the device
 */
int16_t lora_BME280::readS16_LE(byte reg) {
 ...
 */
uint32_t lora_BME280::read24(byte reg) {
  uint32_t value;
 ...
 */
void lora_BME280::readCoefficients(void) {
 ...
 */
bool lora_BME280::isReadingCalibration(void) {
 ...
 */
float lora_BME280::readTemperature(void) {
  int32_t var1, var2;
 ...
 */
float lora_BME280::readPressure(void) {
  int64_t var1, var2, p;
 ...
 */
float lora_BME280::readHumidity(void) {
  readTemperature(); // must be done first to get t_fine
 ...
 */
float lora_BME280::readAltitude(float seaLevel) {
 ...
 */
float lora_BME280::seaLevelForAltitude(float altitude, float atmospheric) {
 ...
 */
uint32_t lora_BME280::sensorID(void) { return _sensorID; }
